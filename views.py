...
from main.engine import get_quest_response, get_decision_response
...
def quest_ajax(request, token):
    return get_quest_response(token, 'quest_ajax.tmpl', True)

def decision_ajax(request, token, dec_hash):
    return get_decision_response(token, dec_hash, 'decision_ajax.tmpl', True)
